...
1. **ล้างคุกกี้เสียในเบราဝ်เซอร์:**
   * ลบ Cookie ที่ชื่อ `next-auth.callback-url` ในเบราว์เซอร์ออก (หรือเปิดหน้าต่างแบบ Incognito เทส)
2. **จัดการเรื่องพอร์ตชน (เลือก 1 วิธี):**
   * **วิธีที่ 1 (แนะนำ):** เปิดระบบ **Services** ใน Windows -> สั่งหยุดการทำงาน (Stop) เซิร์ฟวิสของ PostgreSQL ที่อยู่ในเครื่องออกก่อน เพื่อให้พอร์ต `5432` ว่างและใช้งานผ่าน Docker ได้เต็มที่
   * **วิธีที่ 2:** ถ้าต้องใช้ฐานข้อมูลทั้งสองตัว ให้เปลี่ยนพอร์ตของ Docker ในคำสั่งรันให้เป็น `5433:5432` และแก้ `.env` ในโปรเจกต์เป็นพอร์ต `5433`
3. **แก้ไขไฟล์ `prisma/schema.prisma`:**
   * สั่งปรับแต่งไฟล์ [schema.prisma](file:///F:/project-working/edu-flow/prisma/schema.prisma) ตามจุดที่มีการออกแบบแก้ไขไว้ในหัวข้อที่ 3 แล้วรันคำสั่ง:
     ```bash
     npx prisma db push
     ```
